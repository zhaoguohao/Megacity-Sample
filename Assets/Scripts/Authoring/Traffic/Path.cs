...
namespace Unity.MegaCity.Traffic
{
    ...
    public class Path : MonoBehaviour
    {
        ...
        TrafficSettings _rootObject=null;
        ...
        void Awake()
        {
            if (version == 0)
            {
                ...
                if (children.Length > 0)
                {
                    ...
                    {
                        Waypoints.Add(new RoadWaypoint {position = children[a].transform.position - transform.position});
                    }
                }
        ...
        void InsertNodeAtEnd(int count)
        {
        ...
        void AddNodes(int count,int index)
        {
        ...
        static void DrawGizmos(Path path, GizmoType gizmoType)
        {
            ...
            {
                path._rootObject = path.GetComponentInParent<TrafficSettings>();
                return;
            }

            float segmentation = 1.0f / path._rootObject.pathSegments;
