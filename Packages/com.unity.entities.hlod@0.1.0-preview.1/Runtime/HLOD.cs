...
namespace Unity.Entities.HLOD
{
    ...
    public class HLOD : MonoBehaviour
    {
        ...
        [SerializeField] HLODParent[] m_HLODGroupParents;

        [NonSerialized] internal int _Index = -1;

        public HLODParent[] HLODParents
        {
            get { return m_HLODGroupParents; }
            set { m_HLODGroupParents = value; }
        }

        public int GetLODParentIndex(HLODParent lodParent)
        {
            for (int index = 0; index < m_HLODGroupParents.Length; ++index)
            {
                if (m_HLODGroupParents[index] == lodParent)
                    return index;
            }
            return -1;
        }
        ...
        {
            if (m_HLODGroupParents == null || index >= m_HLODGroupParents.Length || m_HLODGroupParents[index] == null)
                return new LODGroup[0];
            else
                return m_HLODGroupParents[index].GetComponentsInChildren<LODGroup>();
        }
        ...
        static void OnBeforeCull(Camera camera)
        {
            ...
            _Manager.Update(
                Unity.Rendering.LODGroupExtensions.CalculateLODParams(camera), _DidSelectionChange, selection);
