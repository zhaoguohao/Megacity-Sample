...
namespace Unity.Entities.HLOD
{
    struct HLODManager
    {
        ...
        public void Update(in Unity.Rendering.LODGroupExtensions.LODParams lodParams, bool selectionChanged, Transform selection)
        {
            if (!_Data.IsCreated)
                return;
        ...
        void ForceHLODSelection(Transform selection)
        {
            ...
            var lodParents = hlodSelection.HLODParents;
            if (lodParents == null)
                return;
            ...
            for (int p = 0; p != lodParents.Length; p++)
            {
                ...
                if (lodParents[p] != null && selection.IsChildOf(lodParents[p].transform))
                {
        ...
        void Update(in Unity.Rendering.LODGroupExtensions.LODParams lodParams, bool ForceAllChanges, NativeList<LODLevelChange> changes, JobHandle deps)
        {
        ...
        struct UpdateLODChangesJob : IJob
        {
            ...
            public Unity.Rendering.LODGroupExtensions.LODParams LodParams;
            ...
            public void Execute()
            {
                ...
                {
                    var data = Data[i];
                    int lodIndex = Unity.Rendering.LODGroupExtensions.CalculateCurrentLODIndex(
                        data.LODDistances, data.GlobalScale, data.GlobalReferencePoint, ref LodParams);
